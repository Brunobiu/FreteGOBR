...
'- **MarkAsPaid_Form**: { payment_method, proof_file?, notes? }. proof_file MIME ∈ PDF/PNG/JPG/WEBP, size <= 5MB. notes 0..1000 chars.'
...
'5. WHEN o admin adiciona/edita faixa, THE Financeiro_Configuracoes_Page SHALL validar: min_value >= 0, max_value > min_value, pct ∈ [0,50], faixas ordenadas por min_value ASC, sem buracos (max[i] = min[i+1]), sem sobreposicao (max[i] <= min[i+1]), 0..5 faixas.'
...
'7. THE coluna Frete SHALL renderizar link /admin/fretes/<frete_id> com texto <id_curto> · <origin> -> <destination>.'
...
'4. WHEN o admin seleciona arquivo, THE MarkAsPaidModal SHALL validar MIME ∈ {PDF, PNG, JPG, WEBP} e size <= 5MB.'
...
'11. THE admin_repasse_mark_paid RPC SHALL validar is_admin_with_permission(FINANCEIRO_EDIT) e payment_method ∈ enum.'
...
Write-Output ('OK. New length: ' + (Get-Item $path).Length + ' bytes')
