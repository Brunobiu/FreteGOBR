...
- [ ] 1. Fundação: domínios fechados, permissões e geradores de teste
  - [ ] 1.1 Criar `src/services/admin/rastreamento/domain.ts`
  ...
  - [ ] 1.2 Estender `src/services/admin/permissions.ts` (Permission_Matrix)
  ...
  - [ ] 1.3 Escrever `permissions_rastreamento.unit.test.ts`
  ...
  - [ ] 1.4 Criar geradores locais `src/__tests__/admin/rastreamento/_generators.ts`
...
- [ ] 2. Núcleo puro: resumo de jornada, causa do abandono e score de risco
  - [ ] 2.1 Implementar `journeySummary.ts` (`buildJourneySummary(events, nowMs)`)
  ...
  - [ ] 2.2 Escrever `journeySummary.unit.test.ts`
    - Casos concretos e bordas (sem eventos, falhas múltiplas, conversão parcial) + determinismo de
      reexecução. Reusa `_generators.ts`.
    - _Requirements: 5.1, 6.5_
  - [ ] 2.3 Implementar `abandonmentClassifier.ts` (`classifyAbandonmentCause(summary, inactivityDays)`)
  ...
  - [ ] 2.4 Property test CP1 — `cp1_abandonment_classifier.property.test.ts`
  ...
  - [ ] 2.5 Implementar `riskScore.ts` (`calculateRiskScore` + `deriveRiskBand` + `RISK_WEIGHTS`)
  ...
  - [ ] 2.6 Property test CP2 — `cp2_risk_score_bounds.property.test.ts`
    - **Property 2 (CP2): Risk_Score — limites + determinismo**
    - **Validates: Requirements 6.1, 6.2, 6.4, 6.5**
  - [ ] 2.7 Property test CP3 — `cp3_risk_score_monotonic.property.test.ts`
    - **Property 3 (CP3): Risk_Score — monotonicidade não-decrescente**
    - **Validates: Requirements 6.3, 6.5**
  - [ ] 2.8 Property test CP4 — `cp4_risk_band_total.property.test.ts`
    - **Property 4 (CP4): Risk_Band — função total + monotonicidade**
    - **Validates: Requirements 6.6, 6.7**

- [ ] 3. Núcleo puro: derivação de etapa e métricas do funil
  - [ ] 3.1 Implementar `stageDerivation.ts` (`deriveFunnelStage(events)`)
  ...
  - [ ] 3.2 Property test CP5 — `cp5_stage_derivation.property.test.ts`
    - **Property 5 (CP5): Stage_Derivation — domínio fechado + determinismo**
    - **Validates: Requirements 8.2, 4.3**
  - [ ] 3.3 Implementar `funnelMetrics.ts` (`computeFunnelMetrics(counts)`)
  ...
  - [ ] 3.4 Property test CP6 — `cp6_funnel_monotonic.property.test.ts`
    - **Property 6 (CP6): Conversion_Funnel — monotonicidade do funil**
    - **Validates: Requirements 8.1, 8.3**
  - [ ] 3.5 Property test CP7 — `cp7_funnel_metrics_bounds.property.test.ts`
    - **Property 7 (CP7): Funnel_Metrics — limites + complemento + determinismo**
    - **Validates: Requirements 8.4, 8.5, 8.6, 8.7**

- [ ] 4. Núcleo puro: motor de regras, lista em risco, recuperação e exportação
  - [ ] 4.1 Implementar `recoveryRuleEngine.ts` (`decideRecovery(trigger, history, cfg)` + `Anti_Spam_Guard`)
  ...
  - [ ] 4.2 Property test CP8 — `cp8_recovery_engine_determinism.property.test.ts`
    - **Property 8 (CP8): Recovery_Rule_Engine — determinismo + domínio fechado**
    - **Validates: Requirements 9.1, 9.2, 9.3**
  - [ ] 4.3 Property test CP9 — `cp9_anti_spam_guard.property.test.ts`
    - **Property 9 (CP9): Anti_Spam_Guard — invariantes de supressão + idempotência**
    - **Validates: Requirements 9.4, 9.5, 9.6, 9.7, 9.11**
  - [ ] 4.4 Implementar `atRiskList.ts` (`filterAndSortAtRisk(rows, filter)`)
  ...
  - [ ] 4.5 Property test CP10 — `cp10_at_risk_list.property.test.ts`
  ...
  - [ ] 4.6 Implementar `recoveryPerformance.ts` (`computeRecoveryRate` + `canTransitionContactStatus`)
  ...
  - [ ] 4.7 Property test CP11 — `cp11_recovery_rate.property.test.ts`
    - **Property 11 (CP11): Recovery_Rate — limites + progressão monotônica de Contact_Status**
    - **Validates: Requirements 11.2, 11.3, 11.6**
  - [ ] 4.8 Implementar `csvExport.ts`, `messageTemplates.ts` e `trackingFilter.ts`
  ...
  - [ ] 4.9 Property test CP12 — `cp12_csv_roundtrip.property.test.ts`
    - **Property 12 (CP12): CSV Export — round-trip**
    - **Validates: Requirements 7.11**
    - `parseCsv(toCsv(rows))` reproduz as linhas lógicas (campos com `;`, `"`, `\n`, `\r`).
  - [ ] 4.10 Property test CP13 (privacidade) — `cp13_no_pii_leak.property.test.ts`
...
- [ ] 5. Checkpoint — núcleo puro verde
...
- [ ] 6. Migration 124 (`124_admin_rastreamento_inteligente.sql`) + par rollback
  - [ ] 6.1 Esqueleto, guarda defensiva e RBAC
  ...
  - [ ] 6.2 Tabelas, índices e RLS admin-only
  ...
  - [ ] 6.3 RPCs `SECURITY DEFINER`, espelho SQL do motor, composição com `system_alerts`, pg_cron e VERIFY
  ...
  - [ ] 6.4 Par `124_admin_rastreamento_inteligente_rollback.sql` (documentado, não auto-aplicado)
...
- [ ] 7. Edge Function de ingestão
  - [ ] 7.1 Implementar `supabase/functions/tracking-ingest/index.ts`
...
- [ ] 8. Camada de serviço (`src/services/admin/rastreamento.ts`)
  - [ ] 8.1 Erros, mensagens pt-BR e tipos
    ...
    - _Requirements: 15.9_
  - [ ] 8.2 Wrappers de leitura com Partial_Degradation
  ...
  - [ ] 8.3 Wrappers de mutação, delegação ao whatsapp e personalização por IA
  ...
  - [ ] 8.4 Property test CP14 (precedência) — `cp14_permission_precedence.property.test.ts`
  ...
  - [ ] 8.5 Testes de serviço — `rastreamento_service.test.ts`
...
- [ ] 9. Checkpoint — backend + serviço verde
  - Garantir que todos os testes passem (tsc + testes de serviço + CP14 + lint). Tirar dúvidas com o
    usuário se surgirem.

- [ ] 10. UI compacta — componentes, página, rota e sidebar
  - [ ] 10.1 `KpiCard` + `AtRiskTable` + `RecoveryActionsMenu` + exportação CSV
  ...
  - [ ] 10.3 `ConversionFunnelChart` + `RecoveryPerformanceChart` (SVG inline)
  ...
  - [ ] 10.4 `TrackingFilterPopover` + `TrackingAiConfigCard`
  ...
  - [ ] 10.5 `AdminRastreamentoPage` + rota + item de sidebar
...
- [ ] 11. Testes de UI
  - [ ] 11.1 Escrever `rastreamentoUI.test.tsx`
...
- [ ] 12. Testes de integração (CI)
  - [ ] 12.1 Escrever a suíte em `tests/admin/rastreamento/`
...
- [ ] 13. Fechamento — Critical_Modules, Regression_Suite e documentação
  - [ ] 13.1 Registrar Critical_Modules em `tests/coverage.config.ts`
    ...
    - _Requirements: 17.6_
  - [ ] 13.2 Regression_Suite, documentação e verificação final de governança
...
- [ ] 14. Checkpoint final — suíte completa verde
